...
namespace Work_Wave.Areas.Identity.Pages.Account
{
    ...
    {
        private readonly SignInManager<WaveUser> _signInManager;
        private readonly UserManager<WaveUser> _userManager;
        private readonly IUserStore<WaveUser> _userStore;
        ...
        private WaveUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<WaveUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(WaveUser)}'. " +
                    $"Ensure that '{nameof(WaveUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<WaveUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<WaveUser>)_userStore;
        }
    }
}
